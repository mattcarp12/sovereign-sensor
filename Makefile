...
vendor-manifests: ## Download and template third-party manifests for embedding
	...
	mkdir -p internal/controller/manifests
	helm repo add cilium https://helm.cilium.io/ > /dev/null 2>&1
	helm repo update > /dev/null 2>&1
	helm template tetragon cilium/tetragon \
		--namespace kube-system \
		--version $(TETRAGON_VERSION) \
		> internal/controller/manifests/tetragon.yaml
	@echo "✅ Vendored into internal/controller/manifests/tetragon.yaml"
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name sovereign-sensor-builder
	$(CONTAINER_TOOL) buildx use sovereign-sensor-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm sovereign-sensor-builder
	rm Dockerfile.cross
